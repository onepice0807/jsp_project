...
<script>
	...
	function validUserId() {
		...
		if ($('#userId').val().length > 2 && $('#userId').val().length < 9) {
			...
			printErrMsg('userId', '아이디는 3자 이상 8자 이하로 필수 입니다!', true);
		}
	...
	function validUserPwd() {
		...
		if ($('#userPwd').val() != $('#userPwd2').val()) {
			$('#userPwd2').val('');
			$('#userPwd').val('');
			printErrMsg('userPwd', '비밀번호가 맞지 않습니다!', true);
		} else {
			isValid = true;
		}
		...
		<form action="registerMember.mem" method="post" enctype="multipart/form-data">
			...
			<div class="mb-3">
				...
				<div class='codeDiv' style="display:none;">
					<input
					type="text" class="form-control" id="mailcode" placeholder="이메일로 전송된 코드 입력...">
